...
interface CatNode { id: string; x: number; y: number; }
...
// Smaller heart path, centered at (0,0), ~13px tall
const HEART_PATH =
  'M 0,-4 C 0,-4 -1.3,-7 -4.5,-7 C -7,-7 -7,-4 -7,-4 C -7,-1.5 -4,2 0,6 C 4,2 7,-1.5 7,-4 C 7,-4 7,-7 4.5,-7 C 1.3,-7 0,-4 0,-4 Z';

// Crack line that runs through the heart center (matching its proportions)
const CRACK_POINTS = '0,-7 2,-3 -1.5,0 2,3.5 0,6';
...
function ConnectionLine({ conn, cats }: { conn: Connection; cats: CatNode[] }) {
  ...
  // Parent / Kit: stepped orthogonal path (straight down → across → straight down)
  ...
  // Mate / Ex-Mate: straight line with heart in the middle
  ...
  // Rotation angle to orient the heart along the line
  ...
  // ex-mate: dark red, broken heart (single heart with crack)
  return (
    <g>
      ...
      <g transform={`translate(${mx},${my}) rotate(${angle})`}>
        {/* Solid heart */}
        <path d={HEART_PATH} fill="#991b1b" stroke="#7f1d1d" strokeWidth={0.5} />
        {/* White crack through the middle */}
        <polyline
          points={CRACK_POINTS}
          stroke="white"
          strokeWidth={1.5}
...
// ─── Arrow (down-pointing CSS triangle) ──────────────────────────────────────
...
export default function FamilyTreeCanvas() {
  ...
  const catDrag = useRef({ id: null as string | null, startClientX: 0, startClientY: 0, nodeStartX: 0, nodeStartY: 0, moved: false });
  ...
  const onNodeMove = useCallback((e: React.PointerEvent) => {
    ...
    const dx = (e.clientX - startClientX) / scale, dy = (e.clientY - startClientY) / scale;
    if (Math.abs(dx) > 3 || Math.abs(dy) > 3) catDrag.current.moved = true;
    const el = nodeRefs.current[id];
    if (el) { el.style.left = `${nodeStartX + dx}px`; el.style.top = `${nodeStartY + dy}px`; }
  }, []);
  ...
  const onNodeUp = useCallback((e: React.PointerEvent, cat: CatNode) => {
    ...
    if (moved) {
      const scale = transformRef.current.scale;
      const dx = (e.clientX - startClientX) / scale, dy = (e.clientY - startClientY) / scale;
      setCats((prev) => prev.map((c) => c.id === id ? { ...c, x: nodeStartX + dx, y: nodeStartY + dy } : c));
      setActivePopup(null); setAddMenuId(null);
    } else {
  ...
  const addCat = useCallback(() => {
    ...
    setCats((prev) => [...prev, { id: Math.random().toString(36).slice(2, 9), x: cx, y: cy }]);
  }, []);
  ...
  const addRelation = useCallback((fromCatId: string, type: RelationType) => {
    ...
    setCats((prev) => [...prev, { id: newId, x: from.x + ox, y: from.y + oy }]);
    setConnections((prev) => [...prev, { id: Math.random().toString(36).slice(2, 9), fromId: fromCatId, toId: newId, type }]);
    setActivePopup(null);
    setAddMenuId(null);
  }, []);
